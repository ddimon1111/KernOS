/* Duplicate of top-level idt.c for building from src/ */
#include "idt.h"
#include "vga.h"
#include "gui.h"
...
void keyboard_handle_make(uint8_t scancode) {
    if (scancode & 0x80) {
        /* key release */
        if (gui_handle_scancode(scancode)) return;
        return;
    }
...
/* The actual low-level ISR is implemented in assembly (src/irq_keyboard.S)
 * to avoid GCC generating forbidden floating-point/MMX instructions
 * inside an interrupt handler. The assembly stub reads the scancode,
 * calls `keyboard_handle_make(uint8_t)`, sends EOI, and performs `iretq`.
 */
